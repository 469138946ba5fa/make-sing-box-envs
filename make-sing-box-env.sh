...
echo "如果想要全局路由你需要配置路由器 DHCP 下发的 Gateway 强制为本机 IP 并配置 DNS 为任意有效值(比如 223.5.5.5,114.114.114.114,1.1.1.1,8.8.8.8 等等)，不要将 DNS 设置为 172.19.0.1 或 fake-ip 地址"
echo "如果想要旁路由，你需要为单个联网设备配置 Gateway 强制为本机 IP 并配置 DNS 为任意有效值(比如 223.5.5.5,114.114.114.114,1.1.1.1,8.8.8.8 等等)，不要将 DNS 设置为 172.19.0.1 或 fake-ip 地址"
