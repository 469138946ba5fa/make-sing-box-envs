...
echo "如果想要全局路由你需要配置路由器 DHCP 下发的 NetGateway 强制为本机 IP 同时将下发 DNS  为路由器 IP 或修改为任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5），不要设置为 172.19.0.1 或 fake-ip 地址"
echo "如果想要旁路由，你需要为单个联网设备配置 NetGateway 强制为本机 IP 同时将下发 DNS  为路由器 IP 或修改为任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5），不要设置为 172.19.0.1 或 fake-ip 地址"
