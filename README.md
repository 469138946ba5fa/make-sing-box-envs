...
## ⚠️ 注意事项
* 需要提前设置好有线网卡 en0 为固定的 ip gateway dns 比如我的路由器 ip 是 192.168.255.254 内网环境是 192.168.255.0/24 , macmini 设备 ip 是 192.168.255.253 gateway 和 dns 一样配置为路由器 ip 192.168.255.254 这样保证本机可以访问内网
...
* 如需**全局路由**，请将路由器 DHCP 下发Gateway 强制为本机 IP 并配置 DNS 为路由器 ip 或 任意有效值(比如 223.5.5.5,114.114.114.114,1.1.1.1,8.8.8.8 等等)，不要将 DNS 设置为 172.19.0.1 或 fake-ip 地址
* 如需**旁路由**，请将路由器或设备 Gateway 强制为本机 IP 并配置 DNS 为路由器 ip 或 任意有效值(比如 223.5.5.5,114.114.114.114,1.1.1.1,8.8.8.8 等等)，不要将 DNS 设置为 172.19.0.1 或 fake-ip 地址
