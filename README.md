...
* 如需**全局路由**，请将路由器 DHCP 下发网关设置为本机 IP 同时设置下发 DNS  为任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5）或路由器 IP (大概率拦截失败)，不要设置为 172.19.0.1 或 fake-ip 地址
* 如需**旁路由**，请将路由器或设备网关设置为本机 IP 同时设置 DNS  为任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5）或路由器 IP (大概率拦截失败)，不要设置为 172.19.0.1 或 fake-ip 地址
